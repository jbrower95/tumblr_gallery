	...
	var gal = {
		...
		*/
                ...
                */
                ...
                var clickListener = function() {
                	var blogName = $("#blogName").val();
                			...
                			gal.canvas.requestPointerLock();
                	if (!!blogName) {
                		// blogName is not null
                		tumblr.loadRecentPhotoURLs(blogName, 30).then(function(urls) {
                			gal.painting_sources = urls;

                			// load the font we need.
                			gal.fontLoader = new THREE.FontLoader();
                			gal.fontLoader.load("fonts/times.json", function(font) {
                				gal.font = font;
                				// finally, add paintings.
                				gal.addPaintings();
                			});
                		});
                ...
                gal.bgMenu.addEventListener("click", clickListener);
                gal.play.addEventListener("click", clickListener);
		...
		create: function() {
            ...
            textureLoader.load("img/Textures/Floor.jpg", function(texture) {
            	gal.floorText = texture;
		...
		addPaintings: function() {
			...
			// Remove any paintings
			if (gal.paintings && gal.paintings.length > 0) {
				for (var i = 0; i < gal.paintings.length; i++) {
					gal.scene.remove(gal.paintings[i]);
				}
			}

			// Remove any captions:
			if (gal.captions && gal.captions.length > 0) {
				for (var i = 0; i < gal.captions.length; i++) {
					gal.scene.remove(gal.captions[i]);
				}
			}
			...
			gal.num_of_paintings = gal.painting_sources.length;
			gal.paintings = [];
			...
			for(var i = 0; i < gal.num_of_paintings; i++){
				(function(index) {
					var source = gal.painting_sources[i];

                    //https://developer.mozilla.org/en-US/docs/Web/API/HTMLImageElement/Image
					var artwork = new Image();
					artwork.crossOrigin = true;
					artwork.src = source.src;

					var ratiow = 0;
					var ratioh = 0;
                    
					textureLoader.load(artwork.src, function(texture) {

						console.log(texture.image.clientWidth);
						texture.minFilter = THREE.LinearFilter;
						var img = new THREE.MeshBasicMaterial({ map: texture });
						if (renderFont) {	
							textGeo = new THREE.TextGeometry( source.caption, {
								font: gal.font,
								size: 12,
								height: 20,
								material: 0,
						...
						artwork.onload = function(){
							...
							if (renderFont) {
								var tm = new THREE.Mesh(textGeo, textMaterial);
		                        tm.overdraw = true;
	                    	}
							...
							if (index <= Math.floor(gal.num_of_paintings/2)-1) {
	                            ...
	                            if (renderFont) {
	                            	tm.position.set(2.5 * index - 17.5,2,-2.96);
	                        	}
							} else {
								//plane.rotation.z = Math.PI/2;
	                            plane.position.set(2.5 * index - 55 ,2 ,2.96);
	                            if (renderFont) {
	                            	tm.position.set(2.5 * index - 55 ,2 ,2.96);
	                        	}
